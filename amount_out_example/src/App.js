...
function App() {
  ...
  const url = host + "get_amount_out/";

  //jwt token recieved from authorization endpoint
  const token = "076ea2a71fdc713083dc9fca95ddb260e72810bf";

  const outputTokenRef = useRef(null)
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      ...
      amount_in: e.target.amount_in.value,
    ...
    try {
      ...
      setOutputAmount(res.data.amount_in);
  ...
  return (
    <div className="App">
      <h1>Amount Out</h1>
      <p>Get a quote for a given input amount and swap path from a uniswapv2-compatible router</p>
      <form onSubmit={handleSubmit}>
        ...
        <select id="input_token" name="input_token" ref={outputTokenRef}>
        ...
        <label htmlFor="amount_in">amount in: </label>
        {/* giving a default value for demonstration purposes  */}
        <input id='amount_in' name='amount_in' placeholder='Enter the desired amount in after swap' defaultValue={100000000000000000000} />
        <br></br>
      ...
      <p>estimated amount you will recieve after the swap: <b>{outputAmount}</b> {outputTokenRef.current?.options[outputTokenRef.current.selectedIndex].text}</p>
    </div>
  );
}
